...
export interface HatThemeManifestEntry {
  id: string;
  file: string;
  enabled: boolean;
  sensitive?: boolean;
...
export interface HatThemeLoadResult {
  themes: HatTheme[];
  errors: string[];
}
...
export function parseHatTheme(input: unknown): Omit<HatTheme, "sensitive"> | null {
...
export function parseHatManifest(input: unknown): HatThemeManifestEntry[] | null {
  if (
    !isRecord(input)
...
function isManifestEntry(value: unknown): value is HatThemeManifestEntry {
